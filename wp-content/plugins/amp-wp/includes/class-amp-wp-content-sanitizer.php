...
class Amp_WP_Content_Sanitizer {
	...
	*/
	...
	public static function remove_start_point_amp( $url, $default = false ) {
		...
		$prefix  = amp_wp_permalink_prefix();
		$matched = array();
		...
		// If URL was not AMP.
		if ( Amp_WP_Public::SLUG !== $matched[1] ) {
			return $default;
		}
		...
		if ( $body ) {
			...
			if ( $elements->length ) {
				...
				for ( $i = $elements->length - 1; $i >= 0; $i-- ) {
					...
					$action_xhr = '';
					if ( $action ) {
						$parsed_action = wp_parse_url( $action );
	...
	public static function parse_internal_url( $url ) {
		...
		if ( ! $current_url_parsed ) {
			$current_url_parsed = wp_parse_url( site_url() );
		}
